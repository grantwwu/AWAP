...
public class Game {
    ...
    public Optional<Move> updateState(State newState) {
        ...
        state = newState;
        if (newState.getPlayerID().isPresent()) {
            playerID = newState.getPlayerID().get();
        }
    ...
    private Move findMove() {
        ...
        List<Block> blocks = state.getBlocks().get(playerID);
    ...
    private boolean canPlace(Block block, Point p) {
        ...
        for (Point offset : block.getOffsets()) {
            ...
            if (x > N || x < 0 || y < 0 || y > N
                    || getPos(x, y) >= 0
                    || getPos(x, y) == -2
                    || (x > 0 && getPos(x - 1, y) == playerID)
                    || (y > 0 && getPos(x, y - 1) == playerID)
                    || (x < N && getPos(x + 1, y) == playerID)
                    || (y < N && getPos(x, y + 1) == playerID)) {
                return false;
            }

            onAbsCorner = onAbsCorner || q.equals(corner);
            onRelCorner = onRelCorner
                    || (x > 0 && y > 0 && getPos(x - 1, y - 1) == playerID)
                    || (x < N && y > 0 && getPos(x + 1, y - 1) == playerID)
                    || (x > 0 && y < N && getPos(x - 1, y + 1) == playerID)
                    || (x < N && y < N && getPos(x + 1, y + 1) == playerID);
        }
